...
public class Localization {
	...
	Step step = null;
	GeoPosition lastCalibratedGeoPosition = null;
	...
	public void increment(Data data) {
		...
		if (Double.isNaN(speedkmh) && data instanceof GPSData) {
			...
			{
				double[] p = ((GPSData) data).gps;
				lastCalibratedGeoPosition = new GeoPosition(new LatLong(p[0],
						p[1]), data.time);
			}
		}
	...
	private void increment(MotionData data) {
		...
		// position
		GeoPosition newPosition = track.get(track.size() - 1).increment(
				approxVel);
		...
		if (steps.size() > MINSTEPS) {
			...
			if (calibrated) {
				...
				GeoPosition lastGP = track.get(track.size() - 1);
				List<QueryResult> qr = locationIndex.findNClosest(lastGP.lat,
						lastGP.lon, filter);
				GHPoint3D rGHP = qr.get(0).getSnappedPoint();
				lastCalibratedGeoPosition = new GeoPosition(rGHP.lat, rGHP.lon,
						rGHP.ele, data.time);
	...
	public GeoPosition getClosestPosition(GeoPosition p) {
		...
		GeoPosition snappedPosition = new GeoPosition(result.lat, result.lon,
				p.ele, p.time);
